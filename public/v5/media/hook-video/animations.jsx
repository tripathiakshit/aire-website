...
function Stage({
  ...
  // Export hook: deterministic frame-by-frame seek (used by the MP4 renderer)
  React.useEffect(() => {
    window.__seek = (t) => { setPlaying(false); setTime(clamp(t, 0, duration)); };
  ...
  // Persist playhead
  React.useEffect(() => {
    window.__now = time; // dev/test hook: current playhead, updated every frame
